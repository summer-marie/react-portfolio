...
- [ ] Horizontal scroll present at 375px on `/work` (Portfolio) — confirmed pre-existing in the untouched page-interior content (Bootstrap grid/images), not introduced by the Core Layout Navbar/Footer/shell work or the homepage rebuild. Fix when the Work page's interior is rebuilt. (The matching `/about` overflow is now resolved — see the About rebuild task.)
